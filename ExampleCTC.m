...
for ii = 1 : nPts
    ...
    if ii == 1
        % Store the previous state for the first iteration.
        previousQ = currentQ;
    end
    ...
    if ii > 1
        ...
        nbytes = fprintf('%3.0f%%', 100*(ii/(nPts - 1)));
        ...
        [T, X] = ode45(@(t, x) planarArmODE_CTCGravity(t, x, t_local, ...
            jointPos_prescribed, jointVel_prescribed, jointAcc_prescribed), ...
            [0 t_local(end)], X0);

        trajectory = [trajectory; X]; % Store the trajectory
        previousQ = jointPos_prescribed(:,end)'; 
    end
end
...
% Start a timer:
tic
robot.plot(trajectory(1:6:end, 1:3),'trail',{'r', 'LineWidth', 2});
% End the timer:
toc
% Show the duration:
