...
[S, M_home] = make_kinematics_model();
...
for ii = 1 : nPts
    ...
    while errorPose > 1e-5
        ...
        T = fkine(S,M_home,currentQ,'space');
        currentPose = T(1:3, 4);
    end
    ...
    if ii > 1
        ...
        load = 1.0; 
        controller = 'CC'; % Choose the controller: 'PD', 'PID', or 'CTC'
        ...
        %% Compliance Control:
        if strcmp(controller, 'CC')
            [T, X] = ode45(@(t, x) planarArmODE_CCGravity(t, x, q_des, load, S, M_home), [0 tf], X0);
        end
...
if animateRobot
    ...
    robot.plot(frames, 'fps', fps, 'trail', {'r', 'LineWidth', 2}, 'view', [235, 30], 'nobase', 'workspace', [-0.75 0.75 -0.75 0.75 -0.05 0.75], 'movie', movie_name);
    toc
    % Show the duration:
