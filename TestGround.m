...
%% Animate the robot
if animateRobot
    title(sprintf('Testing: %s, Load: %.1f kg', controller, load));
    ...
    if saveMovie
        movie_name = sprintf('movies/testing/test_%s_%.1f_kg.mp4', controller, load);
        ...
        robot.plot(frames, 'fps', fps, 'trail', {'r', 'LineWidth', 2}, 'view', [235, 30], 'nobase', 'workspace', [-0.75 0.75 -0.75 0.75 -0.05 0.75]);
    end
